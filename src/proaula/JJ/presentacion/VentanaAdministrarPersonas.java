...
import proaula.JJ.servicios.IProyectos;
...
public class VentanaAdministrarPersonas extends javax.swing.JDialog {
    ...
    public VentanaAdministrarPersonas(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    public final void Listar() {
        ...
        List<Persona> listPersona =  personaS.listar();
        Object[] profesor = new Object[5];
        modelo = (DefaultTableModel) tblTabla2.getModel();
            for(int i = 0; i <  listPersona.size() ; i++) {
               profesor[0] = listPersona.get(i).getCodigo();
               profesor[1] = listPersona.get(i).getNombres();
               profesor[2] = listPersona.get(i).getIdentificacion();
               profesor[3] = "";
    ...
    public void Agregar() {
        String tipo_persona = (String) jComCargo.getSelectedItem();
        if(tipo_persona.equals("Profesor")){
            Profesor profesor = new Profesor();
            IService SProfesor = new PersonaService();
            profesor.setNombres(txtNomProf.getText());
            profesor.setApellidos("");
            profesor.setIdentificacion(txtId.getText());
            profesor.setTipo_persona(tipo_persona);
            profesor.setCorreo(txtCorreo.getText());
            profesor.setClave(txtId.getText());
            profesor.setDepartamento("");
            if(SProfesor.agregarProfesor(profesor)){
                JOptionPane.showMessageDialog(null, "¡¡Registrado!!  \n Usuario: " + profesor.getIdentificacion() 
    ...
    void modificar() {
        ...
        int codigo = Integer.parseInt(tblTabla2.getValueAt(fila, 0).toString());
        String nombre = (String) tblTabla2.getValueAt(fila, 1).toString();
        //ACTUALIZAR PROFESOR
        Profesor profesor  = new Profesor();
        IService SProfesor = new PersonaService();
        ...
        profesor.setApellidos("");
        profesor.setIdentificacion(txtId.getText());
        profesor.setTipo_persona(jComCargo.getSelectedItem().toString());
        profesor.setCorreo(txtCorreo.getText());
        profesor.setClave("");
        SProfesor.modificarProfesor(profesor);
    }
    
    void eliminar() throws SQLException {
        IService personaService = new PersonaService();
